...
# Function to refresh the page and re-select previous dropdowns
def refresh_and_reselect(driver, district_index, taluka_index, log_file, state_index=0, category_index=0):
    driver.refresh()
    print_and_log_time("Page refreshed", log_file)
...
def update_terminal_output(progress_tracker):
...
# Function to select an option by index with retries
...
def wait_for_plot_info_update(driver, log_file, instance_id, progress_tracker, retries=5):
    for attempt in range(retries):
        try:
...
def scrape_village(instance_id, district_index, taluka_index, progress_tracker, lock, villages, processed_villages):
    ...
    while True:
        ...
        try:
            ...
            for plot_index in range(1, len(plot_select.options)):
                ...
                try:
                    plot_info_text = wait_for_plot_info_update(driver, log_file, instance_id, progress_tracker)
                except TimeoutException:
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=num_instances) as pool:
        pool.starmap(scrape_village, [
            (instance_id, district_index, taluka_index, progress_tracker, lock, villages, processed_villages)
